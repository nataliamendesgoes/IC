...
class LightSwitch(Environment):
    ...
    def switch(self, agt, option: str):	
        ...
        self.print(f'Light is {switch.values}')
        if switch.values == "on" and option == "off":
            self.print(f"{agt} Switching light off")
        elif switch.values == "off" and option == "on":
            self.print(f"{agt} Switching light on")
        else:
            self.print(f"{agt} Made no change by switching {option} - switch is {switch.values}")
