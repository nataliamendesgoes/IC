...
class Buyer(Agent):
    ...
    def search_trip(self, src, prefs):
        ...
        for trip in trips:
            ...
            score, reason = self.evaluate_trip(trip.values,prefs)
            if score > best_score:
                best_score = score
                best_trip = trip
                best_reason = reason
        ...
        if best_score > 2:
            self.print(f"Trip from {best_trip.values['seller']} accepted: {best_trip.values['attributes']}")     
            self.send(best_trip.values['seller'],achieve,Goal("buy",best_trip.values))
        else:
            self.print(f"Trip from {best_trip.values['seller']} rejected: {best_trip.values['attributes']}, asking to improve {best_reason}")
            self.send(best_trip.values['seller'],achieve,Goal("improve",(best_trip.values,best_reason)))
