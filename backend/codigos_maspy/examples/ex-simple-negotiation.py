...
class Seller(Agent):
    ...
    def buy_product(self, src, product):
        ...
        if my_product:
            self.print(f"{src} is Buying {product}:{my_product.values[1]}")
            self.rm(my_product)
        else:
...
class Buyer(Agent):
    ...
    def buy_product(self, src, product, budget):
        ...
        if seller_product and seller_product.values[1] <= budget:
            self.print(f"Accepting {product} for {seller_product.values[1]}. I had {budget}")
            self.send("Seller", achieve, Goal("Buying", product))
        else:
            self.print(f"Rejected {product} for {seller_product.values[1]}. I had {budget}")
