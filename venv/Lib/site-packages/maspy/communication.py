...
class Channel(metaclass=CommsMultiton):
    ...
    def _add_agent(self, agent: 'Agent'):
        ...
        if self.show_exec and self.my_name != "default":
            ...
            tuple_str = "_".join(str(x) for x in agent.tuple_name)
            self.print(f"Connecting Agent {agent_name}:{tuple_str}")
        if self.my_name != "default":
            agent_name = type(agent).__name__
            tuple_str = "_".join(str(x) for x in agent.tuple_name)
            self.logger.info(f'Connecting Agent {agent_name}:{tuple_str}', extra=self.ch_info)
    ...
    def _rm_agent(self, agent: 'Agent'):
        ...
        if self.show_exec:
            agent_name = type(agent).__name__
            tuple_str = "_".join(str(x) for x in agent.tuple_name)
            self.print(f"Desconnecting Agent {agent_name}:{tuple_str}")
        agent_name = type(agent).__name__
        tuple_str = "_".join(str(x) for x in agent.tuple_name)
        self.logger.info(f'Desconnecting Agent {agent_name}:{tuple_str}', extra=self.ch_info)
