...
class Agent: 
    ...
    def add(self, data_type: Belief | Goal | Plan | Iterable[Belief | Goal | Plan], instant: bool = False):
        ...
        if self.running is False:
            instant = False
        formatted_data = self._format_data("Adding Info", data_type=data_type, instant=instant)
        self.logger.debug(f"Adding Info: {formatted_data}", extra=self.agent_info) if self.logging else ...    
        # self.save_cycle_log("Adding Info", formatted_data)
    ...
    def rm(self, data_type: Belief | Goal | Plan | Iterable[Belief | Goal | Plan], instant: bool = False):
        ...
        if self.running is False:
            instant = False
        formatted_data = self._format_data("Removing Info", data_type=data_type, instant=instant)
        self.logger.debug(f"Removing Info: {formatted_data}", extra=self.agent_info) if self.logging else ...
        # self.save_cycle_log("Removing Info", formatted_data)  
    ...
    def test(self, data_type: Belief | Goal, instant: bool = False):
        ...
        if self.running is False:
            instant = False
        formatted_data = self._format_data("Testing Info", data_type=data_type, instant=instant)
        self.logger.debug(f"Testing Info: {formatted_data}", extra=self.agent_info) if self.logging else ...    
        # self.save_cycle_log("Testing Info", formatted_data) 
    ...
    def _instant_plan(self, event: Event):
        ...
        if args is not None:
            if event.data.values_len < 2:
                ...
                ev_args = (event.data._values,)
            formatted_data = self._format_data('Instant Plan', plan, event, ev_args+args)
            self.logger.debug(f"Instant Plan: {formatted_data}", extra=self.agent_info) if self.logging else ...
            # self.save_cycle_log("Instant Plan", formatted_data, event, plans)
